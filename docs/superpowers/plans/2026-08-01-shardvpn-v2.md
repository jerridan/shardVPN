...
def test_ensure_security_group_creates_one_and_adds_no_rules(ec2):
    ...
    with Stubber(ec2) as stub:
        # expected_params omitted, not ANY: Stubber calls expected_params.items(),
        # so a bare ANY raises AttributeError. ANY is only valid as a VALUE
        # inside the dict, as in the create_security_group call below.
...
def should_alert(idle_bytes: int, threshold: int, last_alert: str | None, now: datetime) -> bool:
