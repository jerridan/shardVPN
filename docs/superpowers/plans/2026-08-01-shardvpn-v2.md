...
_TIMESTAMP_RE = re.compile(r"\A\d{1,11}\Z")
...
@pytest.mark.parametrize(
    ("seconds", "expected"),
    [(0, "0m"), (59, "0m"), (60, "1m"), (360, "6m"), (3600, "1h0m"), (8040, "2h14m"), (90000, "1d1h0m")],
)
...
def test_sweep_terminates_an_expired_node():
    factory = make_factory(
        {"eu-west-1": [node("i-0old", launched="2026-07-01T00:00:00Z", expires="2026-07-02T00:00:00Z")]}
    )
    result = sweep_with(factory)
...
from shardvpn import ec2ops
from shardvpn.settings import PARAM_NAMES, Pointer, PointerUnavailable, get_parameter, read_pointer, write_pointer
...
def sweep(
    ...
    for region, instance in found:
        ...
        if is_expired(expires, now):
            ...
            _publish(sns, topic_arn, f"shardvpn: reaped {instance_id} in {region} (expired {expires})")
        else:
...
def lambda_handler(event: dict, context) -> dict:
    ...
    try:
        verify(
            ...
            extract_body(event),
            settings.cached_secret(control_ssm, settings.PARAM_NAMES["signing_secret"], time.time()),
            int(time.time()),
        )
    except AuthError:
...
def _up(control_ssm, body: dict) -> dict:
    ...
    try:
        expires_at = ttl.parse_ttl(
            body.get("ttl") or settings.get_parameter(control_ssm, settings.PARAM_NAMES["default_ttl"]),
            now,
        )
    except ValueError:
