...
No `[dependency-groups]`: CI and the local venv both install the same three tools explicitly, and an unused block is a lie about how the project is built.
...
`actions/checkout@v7` and `actions/setup-python@v7` are both correct (v7.0.1 and v7.0.0, published 2026-07-20).
...
Run: `.venv/bin/pytest tests/test_statusdoc.py -v`
Expected: FAIL — `ModuleNotFoundError`
...
Expected: clean, all tests passing. If `ruff format --check` reports diffs, run `.venv/bin/ruff format .` and re-run.
