...
def lambda_handler(event: dict, context) -> dict:
    ...
    try:
        ...
        # log.error, NOT log.exception: log.exception implies exc_info=True,
        # which appends the full traceback — and a traceback ends with
        # str(exc). botocore's ParamValidationError embeds the offending
        # parameter values, and run_instances carries UserData holding a live
        # Tailscale auth key, so log.exception here would write that key into
        # CloudWatch Logs verbatim. Both halves verified directly.
