...
generate_certificate_authority() {
  ...
  cp ${EASYRSA_PKI}/ca.crt ${BLINK_KEYS}/ca.crt
...
generate_server_key_and_certificate() {
  ...
  cp ${EASYRSA_PKI}/private/${SERVERNAME}.key ${BLINK_KEYS}/${SERVERNAME}.key
  cp ${EASYRSA_PKI}/issued/${SERVERNAME}.crt ${BLINK_KEYS}/${SERVERNAME}.crt
...
generate_client_key_and_certificate() {
  ...
  cp ${EASYRSA_PKI}/private/${CLIENTNAME}.key ${BLINK_KEYS}/${CLIENTNAME}.key
  cp ${EASYRSA_PKI}/issued/${CLIENTNAME}.crt ${BLINK_KEYS}/${CLIENTNAME}.crt
  echo "Client certificate and key pair created successfully."
}

generate_diffie_hellman_params() {
  openssl dhparam -out ${BLINK_KEYS}/dh.pem 2048
  echo "Diffie Hellman parameters generated successfully."
}

generate_hmac_key() {
  openvpn --genkey --secret ${BLINK_KEYS}/ta.key
